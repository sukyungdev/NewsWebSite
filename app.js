...
menuTopic.forEach((item) => {
    item.addEventListener("click", (event) => {
        findNewsByMenuTopic(event);
    });
});
inputBtn.addEventListener("click", (event) => {
    searchByUserInput(event);
});
...
async function requestNewsData() {
    try {
        ...
        let header = new Headers({
            "x-api-key": "f2n548Y_D0VPD4Gr1-MdEBntHSB0t1RHm-FItMMIt0k",
        });
...
const latestHeadLineNews = () => {
    url = new URL(
        "https://api.newscatcherapi.com/v2/latest_headlines?countries=KR&page_size=6"
    );
    requestNewsData();
};
...
const findNewsByMenuTopic = async (event) => {
    ...
    url = new URL(
        `https://api.newscatcherapi.com/v2/latest_headlines?countries=KR&topic=${topic}&page_size=6`
    );
    requestNewsData();
};
...
const searchByUserInput = async (event) => {
    ...
    url = new URL(
        `https://api.newscatcherapi.com/v2/search?q=${userInput}&countries=KR&page_size=6`
    );
    requestNewsData();
};
          ...
          <a href="${item.link}" target="_blank">
              <div class="news-content">
                  <img
                      src="${
                          item.media ||
                          "https://media.istockphoto.com/vectors/no-image-vector-symbol-missing-available-icon-no-gallery-for-this-vector-id1128826884?b=1&k=20&m=1128826884&s=170667a&w=0&h=7tmZDF3-85_rT_7luTYWkXydcRF4ZQ0UWIbW4IwX32E="
                      }"
                      alt="news-img"
                  />
                  ...
                  <span>${moment(item.published_date).fromNow()} - ${
                item.rights || "no rights info"
            }</span>
              </div>
            </a>
          </div>`;
...
const paginationMaker = () => {
    ...
    let firstPageInGroup =
        lastPageInGroup - 4 <= 0 ? (firstPageInGroup = 1) : lastPageInGroup - 4;
    ...
    <li class="page-item">
    <a class="page-link" href="#" onclick="movePage(${
        page - 1
    })" aria-label="Previous">
    ...
    for (let i = firstPageInGroup; i <= lastPageInGroup; i++) {
        paginationRenderHtml += `<li class="page-item ${
            page == i ? "active" : ""
